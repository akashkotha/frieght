...
> **Important**: Your project is a monorepo with both `backend` and `frontend` folders. Railway needs configuration to know which to deploy.
...
4. **Set Environment Variables** (in "Variables" tab):
   ...
   Jwt__Key=YourProductionSecretKey-ChangeThis-MakeItLong
   Jwt__Issuer=FreightERP
   Jwt__Audience=FreightERP
   ```
...
#### Option B: Using Configuration Files (Already Created)

I've created `nixpacks.toml` and `railway.json` in your root directory. These tell Railway how to build your backend.
...
2. **Deploy on Railway**:
   ...
   - It will build from the `backend/FreightERP.API` directory

#### Verify Deployment:

After deployment, visit:
- **Your API**: `https://your-app.railway.app/api/auth/login`
- **Swagger**: `https://your-app.railway.app/swagger`

You should see the Swagger UI or API response.
